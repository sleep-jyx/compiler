...
void translate()
{
    ...
    while (!op.empty())
    {
        ...
        { //规约项，由上一个状态读入一个终结符
            ...
            else if (reduceGrammaIndex >= 18 && reduceGrammaIndex <= 23)
            { //布尔运算符，bop,{"<", "<=", "==", "!=", ">", ">="}
                Symbol E1, E2;
                for (int popTime = 0; popTime < 3; popTime++)
                { //E->E + E 要出栈三次，进栈一次
                ...
                Symbol T = newtemp();
                string bopStr[8] = {"<", "<=", "==", "!=", ">", ">="};
                GEN(bopStr[reduceGrammaIndex - 18], E1.PLACE, E2.PLACE, T);
                semantic.pop(); //更新语义栈
                semantic.pop();
                semantic.push(T.varName); //语义栈保存符号表的变量名就够了，最大的作用就是规约后用来查LR分析表转入下一个状态
                op.top().PLACE = T.PLACE;
            ...
            else if (reduceGrammaIndex >= 24 && reduceGrammaIndex <= 29)
            { //布尔运算符，bop,{"<", "<=", "==", "!=", ">", ">="}
            }
            else if (reduceGrammaIndex == 30)
            { //BF->i
                status.pop();
                op.pop();
